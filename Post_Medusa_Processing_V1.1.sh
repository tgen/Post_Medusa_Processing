...
# Remove Post processing directory on /scratch
...
do
	...
	then
		ssh ${USER}@pnap-data2.tgen.org "if [ -d /scratch/${USER}/post_medusa_processing/${PATIENT_NAME} ] ; then rm -rf /scratch/${USER}/post_medusa_processing/${PATIENT_NAME} ; fi"
        	if [ $? -ne 0 ]
        	then
        		echo
	...
	# Remove all progress tags, analysis dir and Check for any DNAPAIR= lines and start appropriate snpEff and CNA jobs	
