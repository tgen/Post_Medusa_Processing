...
STARTDIR=`pwd`
USER=`whoami`
WGSPBS=/home/tgenref/pecan/post_central_pipe_processing/post_medusa_V1.0/jobScripts/ngs_cna2015_WGS_normal_pairing_V1.pbs
...
do
	...
	qsub -q overflow -v OFILE=${EXOMEPAIR},BEDFILE=${bedFile},NASSAY="${NASSAY}",TASSAY="${TASSAY}",LIBTYPE="${LIBTYPE}",USER="${USER}",PATIENT_NAME="${PATIENT_NAME}",NORMALSAMPLE="${NORMALSAMPLE}",TUMORSAMPLE="${TUMORSAMPLE}",NORMALDAT="${NORMALDAT}",TUMORDAT="${TUMORDAT}",EXOMEPAIR="${EXOMEPAIR}" ${WGSPBS}
